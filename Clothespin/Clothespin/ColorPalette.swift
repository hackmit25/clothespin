import SwiftUI

extension Color {
    // Your custom color palette
    static let sageGreen = Color(hex: "5F6C37")      // Primary green
    static let darkGreen = Color(hex: "273617")      // Dark accent green
    static let cream = Color(hex: "FEFADF")          // Light cream background
    static let warmBrown = Color(hex: "DCA15D")      // Warm brown accent
    static let rust = Color(hex: "BC6C25")           // Rust accent
    
    // Semantic color mappings
    static let primary = sageGreen
    static let primaryDark = darkGreen
    static let background = cream
    static let accent = warmBrown
    static let secondary = rust
    
    // Additional semantic colors
    static let textPrimary = darkGreen
    static let textSecondary = Color.gray
    static let cardBackground = Color.white
    static let border = sageGreen.opacity(0.3)
}

extension Color {
    init(hex: String) {
        let hex = hex.trimmingCharacters(in: CharacterSet.alphanumerics.inverted)
        var int: UInt64 = 0
        Scanner(string: hex).scanHexInt64(&int)
        let a, r, g, b: UInt64
        switch hex.count {
        case 3: // RGB (12-bit)
            (a, r, g, b) = (255, (int >> 8) * 17, (int >> 4 & 0xF) * 17, (int & 0xF) * 17)
        case 6: // RGB (24-bit)
            (a, r, g, b) = (255, int >> 16, int >> 8 & 0xFF, int & 0xFF)
        case 8: // ARGB (32-bit)
            (a, r, g, b) = (int >> 24, int >> 16 & 0xFF, int >> 8 & 0xFF, int & 0xFF)
        default:
            (a, r, g, b) = (1, 1, 1, 0)
        }

        self.init(
            .sRGB,
            red: Double(r) / 255,
            green: Double(g) / 255,
            blue:  Double(b) / 255,
            opacity: Double(a) / 255
        )
    }
}
